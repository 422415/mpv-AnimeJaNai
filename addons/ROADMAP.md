...
| Media output | Versioned owned output API, local/remote inputs, reviewed HTTP receiver, native GPU encoding/audio/muxing and bounded delivery; real Wasm/native tests | Other receiver transports, longer A/V timing and format/hardware matrix |
| Device/service I/O | Pinned approved HTTP/HTTPS and UDP, async requests, protected scoped header credentials, bounded HTTP media reader | TCP/TLS/WebSockets; authorized listeners/discovery; OAuth; additional remote formats and live playlists |
